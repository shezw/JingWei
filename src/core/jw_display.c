...
int jw_display_present_buffer_rects(jw_display_t *display,
                                    jw_buffer_t *buffer,
                                    const jw_rect_t *rects,
                                    int rect_count)
{
    if (!display || !buffer || !display->proxy || !display->proxy->ops ||
        rect_count < 0 || (rect_count > 0 && !rects)) {
        return -1;
    }

    if (display->proxy->ops->commit_rects) {
        return display->proxy->ops->commit_rects(
            display->proxy, buffer, rects, rect_count);
    }
    if (!display->proxy->ops->commit) {
        return -1;
    }
    return display->proxy->ops->commit(display->proxy, buffer);
}
...
int jw_display_present_rects(jw_display_t *display, const jw_rect_t *rects, int rect_count)
{
    ...
    return jw_display_present_buffer_rects(
        display, display->framebuffer, rects, rect_count);
}
